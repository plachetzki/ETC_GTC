...
hlA.ESind1<- tibble::rownames_to_column(highlight_dfA,var="taxa")
hlG.ESind1<- tibble::rownames_to_column(highlight_dfG,var="taxa")
...
pa<-ggtree(atrFilt1,right=T,layout='roundrect')%<+% hlA.ESind1 + geom_tippoint(aes(shape=tissue),color="aquamarine3",size=2) + scale_shape_manual(values=c(1,2)[c("skin","slime gland") %in% hlA.ESind1$tissue],na.translate=F) +  
  geom_tiplab(aes(subset=grepl("Estout",label)==F & isTip),color="black",size=2,offset=0.1) + 
  geom_tiplab(aes(subset=grepl("Estout",label)),color="aquamarine3",size=2,offset=0.1)+ xlim(0,4) +  
...
#TODO repeat with gamma
# repeat with gamma
gtrFilt1<-ape::extract.clade(gtrFilt,node=111)
pg<-ggtree(gtrFilt1,right=T,layout='roundrect')%<+% hlG.ESind1 + geom_tippoint(aes(shape=tissue),color="darkmagenta",size=2) + scale_shape_manual(values=c(1,2)[c("skin","slime gland") %in% hlG.ESind1$tissue],na.translate=F) +  
  geom_tiplab(aes(subset=grepl("Estout",label)==F & isTip),color="black",size=2,offset=0.1) + 
  geom_tiplab(aes(subset=grepl("Estout",label)),color="darkmagenta",size=2,offset=0.1)+ xlim(0,4) +  
  geom_point2(aes(subset = as.numeric(sub(".*/", "", label))> 75 & !isTip),size=1) +
  geom_point2(aes(subset = as.numeric(sub("/.*", "", label))> 75 & !isTip),size=1) 
...
plotMA_ES<-ggplot(res,aes(x=logCPM,y=logFC)) + 
...
cp<-plot_grid(plotMA_ES,rightplots,ncol=2,labels=c("A",""),rel_widths = c(1.5,1))
...
MA_ES<-cowplot::as_grob(plotMA_ES)
save(atrFilt1,gtrFilt1,hlA.ESind1,hlG.ESind1,MA_ES,file="ESind1.RData")
